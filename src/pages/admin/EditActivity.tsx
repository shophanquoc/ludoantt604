...
export default function EditActivity() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    const path = `headers/${Date.now()}.${file.name.split(".").pop()}`;
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const payload = { title, content, image: image || null };
    const { error } = isNew
    ...
    else { toast({ title: isNew ? "Đã tạo hoạt động" : "Đã cập nhật" }); navigate("/admin/activities"); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        <Button variant="ghost" size="icon" onClick={() => navigate("/admin/activities")}><ArrowLeft className="h-4 w-4" /></Button>
        <h2 className="text-lg font-semibold">{isNew ? "Thêm hoạt động" : "Chỉnh sửa hoạt động"}</h2>
      </div>
      <div className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={() => navigate("/admin/activities")}>Hủy</Button>
        </div>
      </div>
    </div>
  );

...
export default function AdminArticles() {
  const [items, setItems] = useState<Tables<"articles">[]>([]);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button asChild size="sm">
          <Link to="/admin/articles/new"><Plus className="mr-1 h-4 w-4" />Thêm mới</Link>
        </Button>
      </div>
          ...
          <div key={item.id} className="rounded-lg border bg-background p-3">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground">{item.category} · {new Date(item.created_at).toLocaleDateString("vi")}</p>
              </div>
              <div className="flex shrink-0 gap-1">
                <Button variant="ghost" size="icon" className="h-8 w-8" asChild>
                  <Link to={`/admin/articles/${item.id}`}><Pencil className="h-3.5 w-3.5" /></Link>
                </Button>
      ...
      <div className="hidden md:block">
        <Table>
          ...
          <TableBody>
            ...
            {items.length === 0 && (
              <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-8">Chưa có bài viết nào</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">Bạn có chắc chắn muốn xóa bài viết này? Hành động này không thể hoàn tác.</p>
          <DialogFooter>
